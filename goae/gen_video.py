...
def gen_interp_video(G, ws, triplane=None, triplane_x=None, mp4=None, seeds=None, shuffle_seed=None, w_frames=60*4, kind='cubic', grid_dims=(1,1), num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', gen_shapes=False, device=torch.device('cuda'), **video_kwargs):
    grid_w = grid_dims[0]
    grid_h = grid_dims[1]
    num_keyframes = 1
    ...
    if triplane is not None:
        if triplane_x is not None:
            _ = G.synthesis(ws=ws[:1], c=c[:1], triplane=triplane, triplane_x=triplane_x, train_forward_2=True, noise_mode='const')
        else:
            _ = G.synthesis(ws=ws[:1], c=c[:1], 
                                            triplane=triplane, forward_triplane=True, noise_mode='const')   
    else:
        try:
            _ = G.synthesis(ws[:1], c[:1]) # warm up
        except:
            _ = G.synthesis(ws[:1], c[:1], forward=True)
        ...
        for yi in range(grid_h):
            for xi in range(grid_w):
                ...
                if entangle == 'conditioning':
                    ...
                    if triplane is not None:
                        if triplane_x is not None:
                            img = G.synthesis(ws=w.unsqueeze(0), c=c[0:1], triplane=triplane, triplane_x=triplane_x, train_forward_2=True, noise_mode='const')[image_mode][0]
                        else:
                            img = G.synthesis(ws=w.unsqueeze(0), c=c[0:1], triplane=triplane,  forward_triplane=True, noise_mode='const')[image_mode][0]
                    else:
                        try:
                            img = G.synthesis(ws=w.unsqueeze(0), c=c[0:1], noise_mode='const')[image_mode][0]
                        except:
                            img = G.synthesis(ws=w.unsqueeze(0), c=c[0:1], forward=True, noise_mode='const')[image_mode][0]
